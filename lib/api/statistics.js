...
var request = require('request'),
    ...
    async = require('async'),
    dns = require('dns'),
    isDomainName = require('is-domain-name');
    
module.exports = function (app) {
    function Blocks () {
        this.maxOffset = 5760; // 8700
        this.maxCount  = 5760; // 24 hours
    ...
    this.getBlocks = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('address') + blocks.url(offset, limit),
                    json : true
    ...
    this.getLastBlock = function (error, success) {
        ...
        request.get({
            url: app.get('address') + blocks.url(0, 1),
            json : true
        }, function (err, resp, body) {
            if (err || resp.statusCode !== 200) {
    ...
    function Peers (locator) {
        ...
        this.isDuplicatePeer = function (arr, p) {
          let flag = false;
        ...
        this.process = function (p, next) {
            ...
            if ('OK' === p.status) {
              if(!this.isDuplicatePeer(this.list.connected, p)) {
                ...
                if(!this.isDuplicatePeer(this.list.disconnected, p)) {
                  this.list.disconnected.push(p);
                }
            }

            //Check for duplicates because ARK API ignore limit/offset
    ...
    this.getPeers = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('address') + peers.url(offset, limit),
                    json : true
                }, function (err, resp, body) {
                    if (err || resp.statusCode !== 200) {
                        return next(err || 'Response was unsuccessful');
