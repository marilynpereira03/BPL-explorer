...
module.exports = function (app) {
    function Active () {
        this.getActive = function (cb) {
            request.get({
                url : app.get('bpl address') + '/api/delegates/?orderBy=rate:asc&limit='+ app.get('delegates'),
                json : true
    ...
    function Standby (n) {
        ...
        this.actualOffset = (isNaN(this.offset)) ? app.get('delegates') : this.offset + app.get('delegates');
    ...
    this.getStandby = function (n, error, success) {
        ...
        request.get({
            ...
            if (err || response.statusCode !== 200) {
                ...
                body.delegates  = parseDelegates(body.delegates);
                body.totalCount = (body.totalCount - app.get('delegates'));
    ...
    this.getNextForgers = function (error, success) {
        request.get({
            url : app.get('bpl address') + '/api/delegates/getNextForgers?limit='+ app.get('delegates'),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
