...
module.exports = function (app) {
    this.getTransaction = function (transactionId, error, success, url) {
        ...
        async.waterfall([
            function (waterCb) {
                request.get({
                    url : app.get('bpl address') + url + transactionId,
                    json : true
    ...
    function Transaction () {
        this.getDelegate = function (forger, cb) {
            request.get({
                url : app.get('bpl address') + '/api/delegates/get?publicKey=' + forger.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
    ...
    this.getUnconfirmedTransactions = function (error, success, transactions) {
        async.waterfall([
            function (cb) {
                request.get({
                    url : app.get('bpl address') + '/api/transactions/unconfirmed',
                    json : true
                }, function (err, response, body) {
                    if (err || response.statusCode !== 200) {
                        return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getLastTransactions = function (error, success) {
        request.get({
            url : app.get('bpl address') + '/api/transactions?orderBy=timestamp:desc&limit=20',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getTransactionsByAddress = function (query, error, success) {
        ...
        if (query.direction === 'sent') {
            directionQuery = 'senderId=' + query.address;
        } else if (query.direction === 'received') {
            // TODO: We should also support type 7 here
            //directionQuery = 'recipientId=' + query.address + '&and:type=0';
            directionQuery = 'recipientId=' + query.address;
        } else {
            //directionQuery = 'recipientId=' + query.address + '&or:senderId=' + query.address;
            directionQuery = 'recipientId=' + query.address + '&senderId=' + query.address;
        }

        async.waterfall([
            function (cb) {
                request.get({
                    url : app.get('bpl address') + '/api/transactions?' + directionQuery + '&orderBy=timestamp:desc' + '&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
                    json : true
                }, function (err, response, body) {
                    if (err || response.statusCode !== 200) {
                        return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getTransactionsByBlock = function (query, error, success) {
        ...
        async.waterfall([
            function (cb) {
                request.get({
                    url : app.get('bpl address') + '/api/transactions?blockId=' + query.blockId + '&orderBy=timestamp:desc' + '&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
                    json : true
                }, function (err, response, body) {
                    if (err || response.statusCode !== 200) {
                        return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    var setDelegateToCache = function (delegate, tmpDelegateCache) {
        ...
        if (delegateCache && !tmpDelegateCache && delegate && delegate.address && delegateCache[delegate.address] === undefined) {
            console.log ('Storing global cache for: ' + delegate.address);
        ...
        if (tmpDelegateCache && delegate && tmpDelegateCache[delegate] === undefined) {
            console.log ('Storing tmp cache for: ' + delegate);
    ...
    var getSenderDelegate = function (transaction, cb, tmpDelegateCache) {
        ...
        request.get({
            url  : app.get ('bpl address') + '/api/delegates/get?publicKey=' + transaction.senderPublicKey,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                transaction.senderDelegate = null;
    ...
    var getRecipientPublicKey = function (transaction, cb, tmpDelegateCache) {
        ...
        request.get ({
            url  : app.get ('bpl address') + '/api/accounts/getPublicKey?address=' + transaction.recipientId,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                transaction.recipientPublicKey = null;
    ...
    var getRecipientDelegate = function (transaction, cb, tmpDelegateCache) {
        ...
        request.get ({
            url  : app.get ('bpl address') + '/api/delegates/get?publicKey=' + transaction.recipientPublicKey,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                transaction.recipientDelegate = null;
