...
module.exports = function (app) {
    function Account () {
        this.validAddress = function (address) {
            return (
                typeof address === 'string' && address.match(/^[B|b]{1}[0-9a-zA-Z]{33}$/g)
            );
        };

        this.getAccount = function (address, cb) {
            request.get({
                url : app.get('bpl address') + '/api/accounts?address=' + address,
                json : true
        ...
        this.getDelegate = function (account, cb) {
            ...
            request.get({
                url : app.get('bpl address') + '/api/delegates/get?publicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getVotes = function (account, cb) {
            ...
            request.get({
                url : app.get('bpl address') + '/api/accounts/delegates/?address=' + account.address,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getVoters = function (account, cb) {
            ...
            request.get({
                url : app.get('bpl address') + '/api/delegates/voters?publicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getIncomingTxsCnt = function (account, cb) {
            ...
            request.get({
                url : app.get('bpl address') + '/api/transactions?recipientId=' + account.address + '&limit=1',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getOutgoingTxsCnt = function (account, cb) {
            ...
            request.get({
                url : app.get('bpl address') + '/api/transactions?senderId=' + account.address + '&limit=1',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getForged = function (account, cb) {
            ...
            request.get({
                url : app.get('bpl address') + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getTopAccounts = function (query, error, success) {
        ...
        request.get({
            url : app.get('bpl address') + '/api/accounts/top?&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
